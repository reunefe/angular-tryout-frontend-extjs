...
folder are mapped to the application's root namespace, 'Angular.tryout'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Angular.tryout"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Angular.tryout' in the same way
as `"Angular.tryout/sass/src"`.
